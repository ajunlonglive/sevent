...
public:
    ChatHandler() : connections(new set<TcpConnection::ptr>) {}
private:
    void onConnection(const TcpConnection::ptr &conn) {
        ...
        if (!connections.unique()) // copy on write
            connections.reset(new set<TcpConnection::ptr>(*connections));
...
private:
    mutex mtx;
    shared_ptr<set<TcpConnection::ptr>> connections;
};
