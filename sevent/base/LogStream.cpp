...
template <typename T> void LogStream::formatInteger(T v) {
    if (buffer.remain() >= maxNumericSize) {
...
LogStream &LogStream::operator<<(double v) {
    if (buffer.remain() >= maxNumericSize) {
        int len = snprintf(buffer.current(), maxNumericSize, "%.12g", v);
        buffer.add(len);
    }
    return *this;
}
...
LogStream &LogStream::operator<<(const char *str) {
    if (str) {
        buffer.append(str, static_cast<int>(strlen(str)));
    } else {
...
LogStream &LogStream::operator<<(const std::string &v) {
    buffer.append(v.c_str(), static_cast<int>(v.size()));
    return *this;
}